// lib/screens/web_form_editor.dart - Part 1: Main Class & State Management (Compact)
...
import '../models/form_permission.dart';
import '../models/user_group.dart';
import '../services/supabase_service.dart';
import '../theme/app_theme.dart';
import 'web_form_field_editor.dart';

class WebFormEditor extends StatefulWidget {
  ...
  @override
  State<WebFormEditor> createState() => _WebFormEditorState();
}

class _WebFormEditorState extends State<WebFormEditor>
  ...
  List<FormPermission> _permissions = [];
  ...
  void _initializeFormData() {
    ...
    _originalIsRecurring = widget.form.isRecurring;
    _originalVisibility = widget.form.visibility;

    // If it's a checklist but times are not set, set defaults
    if (_isChecklist) {
  ...
  Future<void> _loadFormPermissions() async {
    ...
    try {
      _permissions = await _supabaseService.getFormPermissions(widget.form.id);
    } catch (e) {
  ...
  // lib/screens/web_form_editor.dart - Part 2: UI Components & Dialogs (Compact)
  ...
  // lib/screens/web_form_editor.dart - Part 3: Field Management (Compact)
  ...
  // lib/screens/web_form_editor.dart - Part 4: Scheduling Section (Compact)
  ...
  // lib/screens/web_form_editor.dart - Part 5: Date Inputs & Permissions Section (Compact)
  ...
  // lib/screens/web_form_editor.dart - Part 6: User Management & Build Methods (Compact)
  ...
  Future<void> _showUserSelectionDialog(
    ...
    if (result != null && result.isNotEmpty) {
      ...
      try {
        ...
        _showErrorMessage('Error adding users: ${e.toString()}');
      } finally {
        if (mounted) {
          setState(() => _isLoading = false);
        }
  ...
  Future<void> _addGroupToForm() async {
    ...
    if (result != null) {
      ...
      try {
        ...
        _showErrorMessage('Error adding group: ${e.toString()}');
      } finally {
        if (mounted) {
          setState(() => _isLoading = false);
        }
  ...
  // Remove a permission
  Future<void> _removePermission(FormPermission permission) async {
    setState(() => _isLoading = true);

    try {
      if (permission.user_id != null) {
        await _supabaseService.removeFormPermission(
          widget.form.id,
          userId: permission.user_id,
        );
      } else if (permission.group_id != null) {
        await _supabaseService.removeFormPermission(
          widget.form.id,
      ...
      _showErrorMessage('Error removing permission: ${e.toString()}');
    } finally {
      if (mounted) {
        setState(() => _isLoading = false);
      }
  ...
  // lib/screens/web_form_editor.dart - Part 7: Final Build Methods & Update Logic (Compact)

  // Update form method (optimized logic)
  Future<void> _updateForm() async {
    ...
    try {
      ...
      if (mounted) {
        _showErrorMessage('Error updating form: ${e.toString()}');
      }
    } finally {
      if (mounted) {
        setState(() => _isLoading = false);
  ...
  // OPTIMIZED compact update button
