// ENHANCED ARABIC PDF SERVICE WITH PROPER TEXT SHAPING
  ...
  Future<void> _initialize() async {
    ...
    try {
      ...
      try {
        ...
        arabicFontBold = arabicFont;
        _initialized = true;
        debugPrint('Alternative Arabic font loaded successfully');
      } catch (e2) {
        debugPrint('Failed to load any Arabic font: $e2');
...
// Create RTL-aware row for Arabic content
  ...
  Future<File> generateFormResponsePdfWithFullPageImages(
      CustomForm form, FormResponse response) async {
    try {
      await _initialize();
      debugPrint(
          'PDF Service initialized: $_initialized, Arabic support: $hasArabicSupport');
    } catch (e) {
      debugPrint('Error during PDF service initialization: $e');
    }
    ...
    try {
      ...
      // Save PDF
      final fileName = '${form.title}_response_${response.id}.pdf';
      final output = await _savePdfFile(fileName);
      ...
      debugPrint('Error generating PDF content: $e');
      rethrow;
    }
  }

  // Legacy method
  Future<File> generateFormResponsePdf(
      CustomForm form, FormResponse response) async {
    return generateFormResponsePdfWithFullPageImages(form, response);
  }
  ...
  Future<File> _savePdfFile(String fileName) async {
    ...
    if (await file.exists()) {
      try {
        ...
        fileName = '${fileName.split('.').first}_$timestamp.pdf';
  ...
  Future<void> sharePdf(File pdfFile, BuildContext context) async {
    try {
