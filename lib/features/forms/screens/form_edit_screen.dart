// lib/screens/forms/form_edit_screen.dart
...
import 'package:jala_form/features/forms/widgets/form_field_editor.dart';
...
class _FormEditScreenState extends State<FormEditScreen> {
  ...
  late List<FormFieldModel> _originalFields;
  late bool _originalIsChecklist;
  late FormVisibility _originalVisibility;

// Initialize original values
  void _initializeOriginalValues() {
    _originalTitle = widget.form.title;
    _originalDescription = widget.form.description;
    _originalFields = List.from(widget.form.fields);
    _originalIsChecklist = widget.form.isChecklist;
    _originalVisibility = widget.form.visibility;
  }
...
// Check if form has been modified
  bool _hasFormBeenModified() {
    ...
    if (_fields.length != _originalFields.length) return true;
    if (_isChecklist != _originalIsChecklist) return true;
    if (_visibility != _originalVisibility) return true;

    // Check if fields have been modified
    for (int i = 0; i < _fields.length; i++) {
      if (i >= _originalFields.length) return true;
      if (_fields[i].id != _originalFields[i].id ||
          _fields[i].label != _originalFields[i].label ||
          _fields[i].type != _originalFields[i].type ||
          _fields[i].isRequired != _originalFields[i].isRequired) {
        return true;
      }
    }
...
// Track changes in form
  void _markAsModified() {
    if (!_hasUnsavedChanges && _hasFormBeenModified()) {
      setState(() {
...
// Show unsaved changes warning
  Future<bool> _showUnsavedChangesDialog() async {
    if (!_hasFormBeenModified() || _isNavigatingAway) {
      return true;
    }
  ...
  void initState() {
    ...
    // Initialize checklist attributes
    _isChecklist = widget.form.isChecklist;
    _isRecurring = widget.form.isRecurring;
    _recurrenceType = widget.form.recurrenceType ?? RecurrenceType.once;
    _startTime = widget.form.startTime;
    _endTime = widget.form.endTime;
    _startDate = widget.form.startDate;
    _endDate = widget.form.endDate;
    _visibility = widget.form.visibility;

    // Initialize original values and add listeners
    _initializeOriginalValues();
    _titleController.addListener(_markAsModified);
    _descriptionController.addListener(_markAsModified);

    _loadGroups();
    _loadFormPermissions();
  }
  ...
  Future<void> _loadFormPermissions() async {
    try {
      final permissions =
          await _supabaseService.getFormPermissions(widget.form.id);
      if (mounted) {
        setState(() {
          _permissions.clear();
      ...
      debugPrint('Error loading form permissions: $e');
    }
  }
          ...
          actions: [
            Container(
              ...
              child: Material(
                ...
                child: InkWell(
                  onTap: _isLoading ? null : _updateForm,
                  borderRadius: BorderRadius.circular(8),
                  child: Container(
                          ...
                          children: [
                            ...
                            FormBuilderTextField(
                              ...
                              decoration: InputDecoration(
                                ...
                                hintText: 'Enter a description for your form',
                                border: OutlineInputBorder(
                                  borderRadius: BorderRadius.circular(10),
                                  borderSide:
                                      BorderSide(color: Colors.grey.shade300),
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        ...
                        const SizedBox(height: 2),
                        Text(
                          'Form repeats based on schedule',
                          style: TextStyle(
                            fontSize: 13,
                            color: Colors.grey[600],
                          ),
                              ...
                              onPressed: () async {
                                ...
                                try {
                                  await _supabaseService
                                      .removeFormPermission(permission.id);
                                  setState(() {
                                  ...
                                  ScaffoldMessenger.of(context).showSnackBar(
                                    SnackBar(
                                      content: Text(
                                          'Removed ${isUser ? 'user' : 'group'} access'),
                                  ...
                                  ScaffoldMessenger.of(context).showSnackBar(
                                    SnackBar(
                                      content: Text(
                                          'Error removing access: ${e.toString()}'),
              ...
              Container(
                ...
                child: Row(
                  children: [
                    ...
                    Expanded(
                      child: Text(
                        'No users or groups have access yet. Only you can access this form.',
                        style: TextStyle(
                          color: Colors.blue[700],
                          fontSize: 13,
                        ),
  ...
  // User and group management methods (same as in FormBuilderScreen)
...
// For FormEditScreen - Replace the _showUserSelectionDialog method with this:
      ...
      if (result != null) {
        try {
          ...
          if (selectedUsers.isNotEmpty) {
            _addUsersToDatabase(selectedUsers);
          }
        } catch (e) {
  ...
  Future<void> _addUsersToDatabase(
    ...
    try {
      for (final user in selectedUsers) {
        ...
        await _supabaseService.addFormPermission(permission);
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Error adding users: ${e.toString()}'),
          backgroundColor: AppTheme.errorColor,
          behavior: SnackBarBehavior.floating,
        ),
      );
  ...
  Future<void> _addGroupToForm() async {
    ...
    if (result != null) {
      try {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Error adding group: ${e.toString()}'),
            backgroundColor: AppTheme.errorColor,
            behavior: SnackBarBehavior.floating,
          ),
        );
  ...
  Future<void> _updateForm() async {
    if (_formKey.currentState!.saveAndValidate()) {
            ...
            SnackBar(
              content: Row(
                children: [
                  ...
                  const SizedBox(width: 12),
                  const Text(
                      'Please set a start date for your recurring checklist'),
                ],
              ),
              behavior: SnackBarBehavior.floating,
              backgroundColor: AppTheme.errorColor,
      ...
      try {
        ...
        final updatedForm = CustomForm(
          id: widget.form.id,
          title: _titleController.text.trim(),
          description: _descriptionController.text.trim(),
          fields: _fields,
          created_by: widget.form.created_by,
          created_at: widget.form.created_at,
          ...
          timeZone: widget.form.timeZone,
        );

        await _supabaseService.updateForm(updatedForm);
            ...
            SnackBar(
              content: Row(
                children: [
                  const Icon(Icons.check_circle, color: Colors.white),
                  const SizedBox(width: 12),
                  const Text('Form updated successfully'),
                ],
              ),
              behavior: SnackBarBehavior.floating,
              backgroundColor: AppTheme.successColor,
